...
def get_rules_from_git():
	os.system("git clone https://github.com/Yara-Rules/rules.git /tmp/rules")

def list_yara_files():
	all_yara_files = []
	for root, directories, filenames in os.walk("/tmp/rules/malware"):
...
def merge_rules(all_rules):
	with open("/root/malware_rules.yar", 'w') as fd:
